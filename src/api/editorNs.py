...
from src.api.newspaperNs import paper_model
...
editor_model = editor_ns.model('EditorModel', {
    ...
    'newspapers': fields.List(fields.Integer,required=False, help='The list of newspapers that the editor is responsible for')
})
@editor_ns.route('/')
...
class EditorID(Resource):
    ...
    def post(self, editor_id):
        try:
            ...
            for i in editor_ns.payload['newspapers']:
                list1.append(Agency.get_instance().get_newspaper(i))
            updated_editor = Agency.get_instance().update_editor(editor_id, editor_ns.payload['editor_name'], editor_ns.payload['address'], list1)
            return updated_editor.to_dict()
        except ValueError as e:
            abort(404, str(e))
